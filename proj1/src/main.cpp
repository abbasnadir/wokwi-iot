...
void updateDisplays() {
  ...
  switch (state) {
    case NORMAL: {
      ...
      if (elapsed < NORMAL_PHASE_MS) phaseLeft = (NORMAL_PHASE_MS - elapsed) / 1000;
    ...
    case AMBUL_WARNING: {
      ...
      for (int i = 0; i < 4; i++) {
        if (i == IDX_S)
          showSeconds(displays[i], secsLeft);
        else
...
void loop() {
  ...
  if (abs(adc - lastStableADC) > DEADBAND) {
    ...
    if (newAmbul != ambulanceNear) {
      ...
      Serial.print("  dist: ~");
      Serial.print(dist);
      Serial.println(ambulanceNear ? "m  -> AMBULANCE DETECTED" : "m  -> Ambulance cleared");
    }
  }
